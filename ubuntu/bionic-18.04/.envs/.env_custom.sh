...
if [[ -z ${IMAGE_URL} ]]; then
    export IMAGE_URL="https://cloud-images.ubuntu.com/bionic/current/"
fi

# VM
if [[ -z ${VM_NAME} ]]; then
    export VM_NAME="ubuntu-bionic-test"
fi
...
if [[ -z ${VM_CLONE_ID} ]]; then
  export VM_CLONE_ID=2210
fi

if [[ -z ${VM_CLONE_NAME} ]]; then
  export VM_CLONE_NAME="ubuntu-bionic-clone"
fi

if [[ -z ${CLOUD_INIT_TEMPLATE_NAME} ]]; then
  export CLOUD_INIT_TEMPLATE_NAME="cloud-init-ubuntu-bionic-default.yml"
fi
