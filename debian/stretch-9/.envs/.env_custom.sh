...
# OS dependent
## OS
export OS_DISTR_NAME="stretch"

# images
if [[ -z ${IMAGE_NAME} ]]; then
    export IMAGE_NAME="debian-9-generic-amd64-daily.qcow2"
fi

if [[ -z ${IMAGE_URL} ]]; then
    export IMAGE_URL="https://cloud.debian.org/images/cloud/stretch/daily/latest/"
fi

# VM
if [[ -z ${VM_NAME} ]]; then
    export VM_NAME="debian-stretch-test"
fi
...
if [[ -z ${VM_CLONE_ID} ]]; then
  export VM_CLONE_ID=2120
fi

if [[ -z ${VM_CLONE_NAME} ]]; then
  export VM_CLONE_NAME="debian-stretch-clone"
fi
...
if [[ -z ${CLOUD_INIT_TEMPLATE_NAME} ]]; then
  export CLOUD_INIT_TEMPLATE_NAME="cloud-init-debian-stretch-default.yml"
fi
