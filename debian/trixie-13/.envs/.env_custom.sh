...
# OS dependent
## OS
export OS_DISTR_NAME="trixie"

# images
if [[ -z ${IMAGE_NAME} ]]; then
    export IMAGE_NAME="debian-13-generic-amd64-daily.qcow2"
fi

if [[ -z ${IMAGE_URL} ]]; then
    export IMAGE_URL="https://cloud.debian.org/images/cloud/trixie/daily/latest/"
fi

# VM
if [[ -z ${VM_NAME} ]]; then
    export VM_NAME="debian-trixie-test"
fi
...
if [[ -z ${VM_CLONE_ID} ]]; then
  export VM_CLONE_ID=2124
fi

if [[ -z ${VM_CLONE_NAME} ]]; then
  export VM_CLONE_NAME="debian-trixie-clone"
fi
...
if [[ -z ${CLOUD_INIT_TEMPLATE_NAME} ]]; then
  export CLOUD_INIT_TEMPLATE_NAME="cloud-init-debian-trixie-default.yml"
fi
