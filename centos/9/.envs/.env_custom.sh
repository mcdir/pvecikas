...
if [[ -z ${IMAGE_NAME} ]]; then
    export IMAGE_NAME="CentOS-Stream-GenericCloud-x86_64-9-latest.x86_64.qcow2"
fi

if [[ -z ${IMAGE_URL} ]]; then
    export IMAGE_URL="https://cloud.centos.org/centos/9-stream/x86_64/images/"
fi

# VM
if [[ -z ${VM_NAME} ]]; then
    export VM_NAME="centos-9-test"
fi
...
if [[ -z ${VM_CLONE_ID} ]]; then
  export VM_CLONE_ID=2050
fi

if [[ -z ${VM_CLONE_NAME} ]]; then
  export VM_CLONE_NAME="centos-9-clone"
fi
...
if [[ -z ${CLOUD_INIT_TEMPLATE_NAME} ]]; then
  export CLOUD_INIT_TEMPLATE_NAME="cloud-init-centos-9-default.yml"
fi
